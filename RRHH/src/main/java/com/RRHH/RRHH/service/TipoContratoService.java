...
public class TipoContratoService {
    ...
    public TipoContratoDTO bucarPorId(Long id){
        TipoContrato contrato = tipoContratoRepository.findById(id).orElseThrow(() -> new RuntimeException("Tipo de contrato no encontrado!"));
        return convertirADTO(contrato);
    }

    public String eliminar(Long id){
        try{
            TipoContrato tipoContrato = tipoContratoRepository.findById(id).orElseThrow(() -> new RuntimeException("¡Imposible eliminar! El tipo de contrato con ID" + id + "no existe."));
    ...
    public TipoContratoDTO actualizarTipoContrato(Long id, TipoContrato tipoContrato){
        TipoContrato contratoExistente = tipoContratoRepository.findById(id).orElseThrow(() -> new RuntimeException("El tipo de contrato no existe en los registros"));
        if (tipoContrato.getNombre() != null) contratoExistente.setNombre(tipoContrato.getNombre());
        if (tipoContrato.getDescripcion() != null) contratoExistente.setDescripcion(tipoContrato.getDescripcion());
        if (tipoContrato.getModalidad() != null) contratoExistente.setModalidad(tipoContrato.getModalidad());
        if (tipoContrato.getJornada() != null) contratoExistente.setJornada(tipoContrato.getJornada());
        if (tipoContrato.getFechaCreacion() != null) contratoExistente.setFechaCreacion(tipoContrato.getFechaCreacion());
